...
def train_main(hparams):
    ...
    # ----------------------
    # look up embeddings
    # context_embedding = MyLayer(output_dim=1024, trainable=False)(tf.slice(context_ph, [0], [1]))
    #utterance_embedding= MyLayer(output_dim=1024, trainable=False)(tf.slice(utterance_ph, [0], [1]))

    #for batch_num in range(1,hparams.batch_size):

    #context_embedding_custom = MyLayer(output_dim=1024, trainable=False)(tf.slice(context_ph, [batch_num], [1]))
    #utterance_embedding_custom = MyLayer(output_dim=1024, trainable=False)(tf.slice(utterance_ph, [batch_num], [1]))
    #context_embedding = tf.concat([context_embedding,context_embedding_custom],axis=0)
    #utterance_embedding = tf.concat([utterance_embedding,utterance_embedding_custom],axis=0)
    ...
    print("context")
    print(tf.shape(context_ph))
    
    context_embedding = elmo(context_ph,signature="default",as_dict=True)["elmo"]
    utterance_embedding = elmo(utterance_ph,signature="default",as_dict=True)["elmo"]
    context_embedding_summed = tf.reduce_mean(context_embedding, axis=1)
    utterance_embedding_summed = tf.reduce_mean(utterance_embedding, axis=1)
    
    print("elmo")
    print(tf.shape(context_embedding))
    print(tf.shape(utterance_embedding))
    # ----------------------
