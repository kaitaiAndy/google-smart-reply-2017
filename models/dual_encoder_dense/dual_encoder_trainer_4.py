...
def train_main(hparams):
    ...
    # ----------------------
    # look up embeddings
    context_embedding_custom0 = MyLayer(output_dim=1024, trainable=True)(tf.slice(context_ph, [0], [1]))
    utterance_embedding_custom0 = MyLayer(output_dim=1024, trainable=True)(tf.slice(utterance_ph, [0], [1]))
    print('context')
    ...
    for batch_num in range(1,hparams.batch_size):
        ...
        context_embedding_custom0 = tf.concat([context_embedding_custom0,context_embedding_custom],
                                      axis=0)
        utterance_embedding_custom0 = tf.concat([utterance_embedding_custom0,utterance_embedding_custom],
                                      axis=0)
...
#print(tf.shape(context_embedding_summed))
    ...
    # ----------------------
    model, _, _, pred_opt = dot_semantic_nn(context=context_embedding_custom0,
